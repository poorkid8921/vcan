    ...
    public void handleFlightDesync() {
        ...
        if (confirmed && !unconfirmed && !joined) {
            if (++this.flightDesyncTicks > Config.DESYNC_MAX_TICKS) {
                Bukkit.getScheduler().runTask(Anticheat.INSTANCE.getPlugin(), () -> this.data.getPlayer().kickPlayer(ColorUtil.translate(Config.DESYNC_KICK_MESSAGE)));
                if (!Config.DESYNC_STAFF_KICK_MESSAGE.equals("")) {
                    Anticheat.INSTANCE.getAlertManager().sendMessage(Config.DESYNC_STAFF_KICK_MESSAGE.replaceAll("%ticks%", Integer.toString(this.flightDesyncTicks)).replaceAll("%player%", this.data.getPlayer().getName()));
                }
    ...
    private void confirmTransactionConnection() {
        ...
        if (delay > Config.TRANSACTION_MAX_DELAY && !this.transactionKicked) {
            ...
            if (transactionBuffer > 5.0) {
                Bukkit.getScheduler().runTask(Anticheat.INSTANCE.getPlugin(), () -> this.data.getPlayer().kickPlayer(ColorUtil.translate(Config.TRANSACTION_KICK_MESSAGE)));
                if (!Config.TRANSACTION_STAFF_ALERT_MESSAGE.equals("")) {
                    Anticheat.INSTANCE.getAlertManager().sendMessage(Config.TRANSACTION_STAFF_ALERT_MESSAGE.replaceAll("%delay%", Long.toString(delay)).replaceAll("%player%", this.data.getPlayer().getName()));
                }
        ...
        if (delay > Config.KEEPALIVE_MAX_DELAY && !this.keepAliveKicked) {
            ...
            if (keepAliveBuffer > 5.0) {
                ...
                if (!Config.KEEPALIVE_STAFF_ALERT_MESSAGE.equals("")) {
                    Anticheat.INSTANCE.getAlertManager().sendMessage(Config.KEEPALIVE_STAFF_ALERT_MESSAGE.replaceAll("%delay%", Long.toString(delay)).replaceAll("%player%", this.data.getPlayer().getName()));
                }
    ...
    private void handleMaxPingKick() {
        ...
        if (this.keepAlivePing > Config.MAX_PING && !this.maxPingKicked) {
            if (++this.maxPingBuffer > Config.MAX_PING_KICK_TICKS) {
                Bukkit.getScheduler().runTask(Anticheat.INSTANCE.getPlugin(), () -> this.data.getPlayer().kickPlayer(ColorUtil.translate(Config.MAX_PING_KICK_MESSAGE)));
                if (!Config.MAX_PING_KICK_STAFF_ALERT_MESSAGE.equals("")) {
                    Anticheat.INSTANCE.getAlertManager().sendMessage(Config.MAX_PING_KICK_STAFF_ALERT_MESSAGE.replaceAll("%ping%", Long.toString(this.keepAlivePing)).replaceAll("%player%", this.data.getPlayer().getName()));
                }
