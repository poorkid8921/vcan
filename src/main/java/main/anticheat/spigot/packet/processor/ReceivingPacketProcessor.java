package main.anticheat.spigot.packet.processor;
...
public class ReceivingPacketProcessor {
    ...
    public void handle(final PlayerData data, final Packet packet, final PacketPlayReceiveEvent event) {
        ...
        for (final AbstractCheck check : data.getChecks()) {
            ...
            // setback logic
            if (data.isSetback()) {
                data.setSetback(false);

                if (event != null) {
                    // check#setback isn't called due to its need to be called in the check
